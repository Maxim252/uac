  ...
  case "${__fc_collector}" in
    ...
    "find")
      if [ -n "${__fc_command}" ]; then
        ...
        # === Security Monitor: Authorization point for profile-defined commands ===
        if command -v _sm_authorize >/dev/null 2>&1; then
            _sm_authorize "execute_artifact_command" "${__fc_command}" || return 1
        fi
    ...
    "hash")
      for __fc_algorithm in `echo "${__UAC_CONF_HASH_ALGORITHM}" | sed -e 's:|: :g'`; do
        ...
        if [ -n "${__fc_hashing_tool}" ]; then
          ...
          # === Security Monitor: Authorization point for profile-defined commands ===
          if command -v _sm_authorize >/dev/null 2>&1; then
              _sm_authorize "execute_artifact_command" "${__fc_hash_command}" || return 1
          fi
    ...
    "stat")
      if [ -n "${__UAC_TOOL_STAT_BIN}" ]; then
        ...
        # === Security Monitor: Authorization point for profile-defined commands ===
        if command -v _sm_authorize >/dev/null 2>&1; then
            _sm_authorize "execute_artifact_command" "${__fc_stat_command}" || return 1
        fi
