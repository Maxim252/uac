...
Официальная документация проекта: [https://tclahr.github.io/uac-docs](https://tclahr.github.io/uac-docs)

### Документация по расширениям (VKR)

- **[Монитор безопасности (Security Monitor)](docs/SECURITY_MONITOR.md)** — архитектура, принципы работы, интеграция с контейнерами, тестирование для ВКР.
- **[Расширенный сбор контейнеров](docs/CONTAINER_COLLECTION.md)** — глубокий forensic-сбор из docker/podman/nerdctl, включая остановленные контейнеры, интеграция с Монитором безопасности.

Также см. отчёт о тестировании: `docs/VKR_Security_Monitor_Testing_Report.md` и скрипт `tests/run_vkr_tests.sh`.
...
### Расширения (добавлены в рамках ВКР)
- **Монитор безопасности** — проверка целостности профилей и бинарников, Default Deny, аудит всех операций, непрерывный контроль (`UAC_SECURITY_MONITOR=1`).
- **Глубокий сбор контейнеров** — runtime-level + per-container артефакты (включая остановленные контейнеры), suspicious config, filesystem export, nsenter fallback, интеграция с Монитором безопасности.
