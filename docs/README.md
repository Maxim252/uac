...
| [CONTAINER_DATA_REFERENCE.md](CONTAINER_DATA_REFERENCE.md) | **Полные таблицы + детальный разбор содержимого** всех артефактов для всех рантаймов (docker/podman/nerdctl/crictl + LXC/pct/jails/zones). Что именно лежит внутри inspect.json, hostconfig.json, suspicious_config.txt, **CRIU checkpoint** (checkpoint.tar.gz + checkpoint_info.txt), criu_version.txt, proc_*.txt, journal-логов, uid_map и т.д. |
