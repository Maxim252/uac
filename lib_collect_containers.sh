...
_collect_containers() {
    ...
    for __cc_runtime in $__cc_runtimes; do
        ...
        if ! "${__cc_runtime}" ps -q >/dev/null 2>&1; then
            _log_msg WRN "Runtime контейнеров '${__cc_runtime}' обнаружен, но демон не отвечает"
            continue
        fi

        _log_msg INF "Обнаружен runtime контейнеров: ${__cc_runtime}"
        printf "[+] Обнаружен runtime контейнеров: %s\n" "${__cc_runtime}" >&2
...
' "${__cc_cid}" 2>/dev/null > "${__cc_cdir}/suspicious_config.txt" || true
            ...
            if [ "${__cc_export_max_mb}" -gt 0 ] 2>/dev/null; then
                ...
                __cc_size_mb=0
                if [ -n "${__cc_size}" ] && [ "${__cc_size}" -gt 0 ] 2>/dev/null; then
        ...
        if [ "${__cc_runtime_count}" -gt 0 ]; then
            ...
            _log_msg INF "Сбор контейнеров завершён для ${__cc_runtime}: ${__cc_runtime_count} контейнер(ов)"
        fi
    done
    ...
    if [ "${__cc_total}" -gt 0 ]; then
        __UAC_CONTAINER_SUMMARY="Рантаймы:${__cc_runtimes_used} | Всего: ${__cc_total} (вкл. остановленные)"
        printf "[+] Собрано из %d контейнеров (рантаймы:%s)\n" "${__cc_total}" "${__cc_runtimes_used}" >&2
    else
